...
<label for="t_usr">Логин пользователя</label>
<input type="text" class="form-control" id="t_usr" placeholder="Логин пользователя">
</div>
<div class="form-group">
...
<br /><br />
<?php
$st = "SELECT id,theme,sta,log FROM `t_ticket_name` INNER JOIN t_users ON t_ticket_name.usr = t_users.uid ORDER BY t_ticket_name.dt DESC";
...
<td><a href="/m_admin/support/<?php echo $row['id']; ?>"><?php echo htmlspecialchars($row['theme']); ?></td>
<td><?php if ($row['sta'] == 0) { ?>Открыт<?php } else { echo '<font color="green">Новые сообщения</font>'; } ?></td>
...
<?php } else { echo '<div class="alert alert-info alert-dismissable p-l-40"><i class="md md-info"></i> Нет обращений.</div>'; } ?>
...
$.ajax({
type: 'POST',
url: '/actions/a_ticket_del.php',
data: {'id': vrdel, 'token': '<?php echo $token; ?>'},
cache: false,
success: function(result){
if (result == '1') {
$(location).attr('href','/m_admin/support');
}
if (result == '3') {
...
$.ajax({
type: 'POST',
url: '/actions/a_ticket_new.php',
data: {'usr': usr, 'th': th, 'msg': msg, 'token': '<?php echo $token; ?>'},
cache: false,
success: function(result){
if (result == '0') {
...
if (result == '4') {
$.jGrowl('Пользоатель не найден', { theme: 'growl-error' });
}
}
});
});
...
<?php } else {
$tget = intval($_GET['id']);
$nowt = mysqli_fetch_assoc(mysqli_query($connect_db, "SELECT id,usr,theme,log FROM t_ticket_name INNER JOIN t_users ON t_ticket_name.usr = t_users.uid WHERE id='$tget' LIMIT 1"));
if(empty($nowt['id'])) { header ("Location: /m_admin/support"); exit; }
mysqli_query($connect_db, "UPDATE t_ticket_name SET sta = '0' WHERE id='$tget' LIMIT 1");
$onl = mysqli_num_rows(mysqli_query($connect_db, "SELECT id FROM t_online WHERE uid='$nowt[usr]' LIMIT 1"));
?>
<div class="block" style="margin: 0; width: auto;">
<div class="chat-member-heading clearfix">
<h5 class="pull-left"><?php echo htmlspecialchars($nowt['theme']); ?> <small>&nbsp;/&nbsp; <?php echo $nowt['log']; ?> <?php if ($onl == 0) { echo 'офлайн'; } else { echo 'онлайн'; } ?></small></h5>
</div>
