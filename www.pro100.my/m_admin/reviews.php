<?php
$pname = 'Отзывы';
$pkey = 'отзывы';
$pdesc = 'Отзывы';
include('../inc/top_menu.php');
if(!USER_LOGGED || $u_id != $admin_uid) { header ("Location: /"); exit; }
?>
<!-- Контент -->

<div class="row">

<div class="panel panel-default">
<div class="panel-heading">
<h3 class="panel-title">Отзывы</h3>
</div>
<div class="panel-body">

<?php
$st = "SELECT id,msg,log FROM `t_rev` INNER JOIN t_users ON t_rev.usr = t_users.uid WHERE st = '0' ORDER BY id ASC";
$qt = mysqli_query($connect_db, $st);
$ht = mysqli_num_rows($qt);
if ($ht > 0) {
?>
<div class="table-responsive">
<table class="table table-hover">
<thead>
<tr align="center">
<th>Пользоатель</th>
<th>Отзыв</th>
<th>Действия</th>
</tr>
</thead>
<tbody>
<?php
while($row = mysqli_fetch_array($qt)) {
?>
<tr>
<td><?php echo $row['log']; ?></td>
<td><?php echo nl2br(htmlspecialchars($row['msg'])); ?></td>
<td>
<a class="btn btn-info btn-sm waves-effect waves-light" href="javascript://" onclick="review_ok('<?php echo $row['id']; ?>')"><i class="fa fa-check"></i></a> 
<a class="btn btn-danger btn-sm waves-effect waves-light" href="javascript://" onclick="review_del('<?php echo $row['id']; ?>')"><i class="fa fa-close"></i></a>
<td>
</tr>
<?php } ?>
</tbody>
</table>
</div>
<?php } else { echo '<div class="alert alert-info alert-dismissable p-l-40"><i class="md md-info"></i> Нет отзывов, ожидающих модерации.</div>'; } ?>

<div id="review_ok" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="balance_out_label" aria-hidden="true">
<div class="modal-dialog">
<div class="modal-content">
<div class="modal-header">
<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
<h4 class="modal-title" id="balance_out_label">Одобрение</h4>
</div>
<div class="modal-body">
<div class="alert alert-info alert-dismissable p-l-40 margnone">
<i class="md md-info"></i> Вы уверены что хотите одобрить отзыв?
</div>
</div>
<div class="modal-footer m-t-20">
<button type="button" class="btn btn-default waves-effect" data-dismiss="modal">Отменить</button>
<button id="btn_ok" class="btn btn-info waves-effect waves-light">Одобрить</button>
</div>
</div>
</div>
</div>

<div id="review_del" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="balance_out_label" aria-hidden="true">
<div class="modal-dialog">
<div class="modal-content">
<div class="modal-header">
<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
<h4 class="modal-title" id="balance_out_label">Удаление</h4>
</div>
<div class="modal-body">
<div class="alert alert-info alert-dismissable p-l-40 margnone">
<i class="md md-info"></i> Вы уверены что хотите удалить отзыв?
</div>
</div>
<div class="modal-footer m-t-20">
<button type="button" class="btn btn-default waves-effect" data-dismiss="modal">Отменить</button>
<button id="btn_del" class="btn btn-danger waves-effect waves-light">Удалитть</button>
</div>
</div>
</div>
</div>

<script type='text/javascript'>
var vrok;
function review_ok(id){
vrok = id;
$('#review_ok').modal('show');
};

var vrdel;
function review_del(id){
vrdel = id;
$('#review_del').modal('show');
};

$(function() {

$('#btn_ok').click(function(){
$.ajax({
type: 'POST',
url: '/actions/a_review_ok.php',
data: {'id': vrok, 'token': '<?php echo $token; ?>'},
cache: false,
success: function(result){
if (result == '1') {
$(location).attr('href','/m_admin/reviews');
}
if (result == '3') {
$.jGrowl('Ошибка', { theme: 'growl-error' });
}
},
error: function(){
$.jGrowl('Ошибка', { theme: 'growl-error' });
}
});
});

$('#btn_del').click(function(){
$.ajax({
type: 'POST',
url: '/actions/a_review_del.php',
data: {'id': vrdel, 'token': '<?php echo $token; ?>'},
cache: false,
success: function(result){
if (result == '1') {
$(location).attr('href','/m_admin/reviews');
}
if (result == '3') {
$.jGrowl('Ошибка', { theme: 'growl-error' });
}
},
error: function(){
$.jGrowl('Ошибка', { theme: 'growl-error' });
}
});
});

});
</script>

</div>
</div>
</div>

<!-- /Контент -->
<?php include('../inc/bottom_menu.php'); ?>