<?php

namespace incl\pro100\Def;


class LangLibAssistant{

    const ARR_ASSISTANT=[
        'ru'=>[
            'name'=>'ПОМОЩНИК',
            'no_message'=>'Для Вас нет сообщений...'
        ],
        'en'=>[
            'name'=>'ASSISTANT',
            'no_message'=>'No messages for You...'
        ]
    ];




    const ARR_ASSISTANT_LES=[
        'ru'=>[
            'les0'=>'Здравствуйте! Приветствуем Вас на проекте. Я помогу Вам разобраться с системой, в этом нет ничего сложного.',
            'les1'=>'<b>Шаг 1.</b> Заполните в профиле контактные и платёжные данные. Это нужно, чтобы Ваши партнёры могли связаться с Вами. <a href="/cabinet/profile">Заполнить</a>...',
            'les2'=>'<b>Шаг 2.</b> Перейдите на страницу Пополнить, затем Повысить уровень и система переведёт сумму (соответствующую стоимости покупаемого уровня) Вашему куратору данного уровня. <a href="/cabinet/up">Перейти</a>',
            'les3'=>'<b>Шаг 3.</b> После перевода уровень будет повышен автоматически.',
            'les4'=>'<b>Шаг 4.</b> Теперь Вы можете приглашать партнёров по своей реферальной ссылке и получать денежные подарки.',
            'les5'=>'Вы освоились на проекте и поняли принцип работы. Приглашайте партнёров, заполняйте линии и не забывайте вовремя повышать свой уровень. Удачной работы!'
        ],
        'en'=>[
            'les0'=>'Hello, welcome to the project. I\'ll help You with the system, there\'s nothing complicated about it.',
            'les1'=>'<b>Step 1.</b> Fill in Your contact and payment information in Your profile. This is for Your partners can contact You. <a href="/cabinet/profile">Fill in</a>...',
            'les2'=>'<b>Step 2.</b> Go to the “Replenish page”, then go to “Upgrade level” and the System will transfer the amount (corresponding to the cost of the purchased level) to Your curator of that level. <a href="/cabinet/up">Go to</a>...',
            'les3'=>'<b>Step 3.</b> The level will be raised automatically after transfer.',
            'les4'=>'<b>Step 4.</b> Now You can  invite partners using Your referral link and receive cash gifts.',
            'les5'=>'You have mastered the project and understand the principle of work. Invite partners, fill in the lines, and don\'t forget to raise your level in time. Have a great job!'
        ]
    ];

}